...
| --- | --- | --- | --- | --- |
| Raw idea request | Idea-only request (no URL) produces article option(s), with no fabricated source list. | Confirmed in `TESTING_FINDINGS.md` Session 1 #7: a raw-idea-only request correctly scored source grounding 1/5, hit the revision cap, and escalated to human review instead of pretending it had sources. | **Pending final live reconfirm** — verified pre-deploy; re-run on the live URL and update this row from `LIVE_TEST_SESSION.md` §1 before submitting. | Confirms the evaluator discriminates rather than rubber-stamping (the exact failure EDGE_CASES.md #17 warns about). |
| URL-based request | One real, scrapeable URL is extracted and used; the article's claims trace to it. | Confirmed in `TESTING_FINDINGS.md` Session 1: extraction works through the real intake form after the multi-URL form fix. | **Pending final live reconfirm** — see `LIVE_TEST_SESSION.md` §2. | Form originally only supported one URL input; fixed to support multiple. |
| Research and source grounding | Mixed good/dead sources: failed source shown as failed with a reason; article traces only to usable sources. | Confirmed in `TESTING_FINDINGS.md` Session 2/3: failed scrapes get their own `sources` row with `status=failed` + reason; discarded sources get a specific `unusable_reason`, not a generic one. All-sources-fail case (EDGE_CASES #6) was observed actually happening, not just anticipated, and is now caught by the `is_ungroundable()` guard after one attempt instead of the full revision cap. | **Pending final live reconfirm** — see `LIVE_TEST_SESSION.md` §3. | Real cost data from this failure mode: $0.1651 / 8 Claude calls before the guard; the guard now stops this after 1 attempt. |
| Evaluation and revision loop | Evaluation returns pass/revise/reject + per-criterion notes; revision history preserved. | Confirmed structurally (`article_drafts` append-only + `parent_draft_id` lineage) and behaviorally in Session 1/2 (evaluator correctly drove multiple revision attempts before escalating). | **Pending final live reconfirm** — see `LIVE_TEST_SESSION.md` §4. | Malformed-Claude-response `KeyError`s (Session 3 #5) were turned into typed, human-readable errors via `claude/outputs.py::require_fields()`. |
| Human approval before publishing | Cannot reach queued/published state without an explicit approve; a "selected" option is never treated as "approved." | Designed and code-reviewed (`REVIEWABLE_DRAFT_STATUSES`, `option_selected` vs `approved` kept as separate fields) — **not yet explicitly re-walked live** on this deploy. | **Not yet verified live — do this first.** See `LIVE_TEST_SESSION.md` §5; this is the single most safety-critical scenario and should not ship as "pending" at submission time. | |
| Channel formatting | LinkedIn/X/newsletter genuinely differ (not one article, three fonts); no leaked markdown; over-limit output is caught, not silently approved. | Hard guards added (`claude/quality_guards.py`): real word/char counts recomputed per channel independent of the model's self-reported pass; a violation marks that adaptation `failed`, never auto-queued. Not yet walked live side-by-side. | **Not yet verified live** — see `LIVE_TEST_SESSION.md` §6. | |
| Publishing or scheduling | Approved content lands in a visibly-labeled queue state, honestly distinguished from "actually posted." | `ready_to_publish` intermediary status added (Session 1) specifically so the mock adapter stops implying a final "published" state. Not yet walked live. | **Not yet verified live** — see `LIVE_TEST_SESSION.md` §7. | v1 publishing is a queue, not a real LinkedIn/X/email integration — stated here and in the one-pager, not discovered later. |
| Failure handling | A broken step surfaces a human-readable error in the UI and a matching `stage_events` log entry. | Confirmed in `TESTING_FINDINGS.md` Session 3: raw Postgres/`KeyError` exceptions were being shown verbatim; fixed with pre-checks (duplicate-revision race) and `require_fields()` (malformed Claude response). | **Pending final live reconfirm** — see `LIVE_TEST_SESSION.md` §8. | Also fixed this session (2026-09-17): a real security bug — `_is_allowlisted()` built a PostgREST filter by string-interpolating the raw email, a filter-injection risk. Replaced with two parameterized `.eq()` queries; regression tests added (`tests/test_auth.py`); full suite 92/92 passing. |
