...
def on_failure(*, attempts: int, max_attempts: int, error: str, retryable: bool = True) -> dict:
    ...
    `retryable=False` (claude/errors.py — a permanent Anthropic error like an
    invalid key or exhausted billing credit) exhausts immediately regardless
    of `attempts`: retrying something that fails identically every time only
    burns the exponential backoff (up to 8+ minutes) before landing at the
    same dead end attempts would have reached anyway."""
