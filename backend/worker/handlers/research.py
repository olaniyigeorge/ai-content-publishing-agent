...
def handle_research(job: dict) -> None:
    ...
    if source_rows:
        ...
        by_id = {s["source_id"]: s for s in selection["sources"]}
        for row in source_rows:
            picked = by_id.get(row["id"])
            if not picked:
                continue
            if not picked["usable"]:
                db.table("sources").update({"status": SourceStatus.DISCARDED.value}).eq("id", row["id"]).execute()
                continue
            db.table("sources").update(
                {
                    "excerpt_selected": picked["excerpt_selected"],
                    "relevance_notes": picked["relevance_notes"],
